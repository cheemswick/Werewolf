  ...
  io.on('connection', (socket) => {
    ...
    socket.on('room:join', ({ code, playerName } = {}) => {
      try {
        ...
        if (result.error) { socket.emit('room:join_error', result.error); return; }
        ...
        console.log(`[room] ${socket.id} vào phòng ${room.code}`);
      } catch (e) {
    ...
    socket.on('game:start', () => {
      ...
      if (!ok) {
        ...
        socket.emit('error', 'Không thể bắt đầu game.');
      } else {
