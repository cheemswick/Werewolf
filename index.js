...
const ROLES = {
  ...
  THIEF:      'thief',
  // Role mới
  FOOL:       'fool',        // Thằng Ngốc
  ELDER:      'elder',       // Già Làng
  ARSONIST:   'arsonist',    // Kẻ Phóng Hỏa
  BLACK_WOLF: 'black_wolf',  // Sói Đen
};
...
// Round 1: Ăn Trộm → Sói → Bảo Vệ → Phù Thủy → Tiên Tri → Cupid → Sói Trắng → Kẻ Phóng Hỏa
// Round 2+:          Sói → Bảo Vệ → Phù Thủy → Tiên Tri → Sói Trắng → Kẻ Phóng Hỏa
const NIGHT_PHASES = {
  THIEF:      'night_thief',
  ...
  BLACK_WOLF: 'night_black_wolf',  // Sói Đen convert người
  ARSONIST:   'night_arsonist',    // Kẻ Phóng Hỏa tẩm xăng/đốt
};

// Role thuộc phe Sói (bầy cắn người)
const WOLF_ROLES = [ROLES.WEREWOLF, ROLES.WOLF_CUB, ROLES.WHITE_WOLF, ROLES.BLACK_WOLF];
...
function buildRoleList(playerCount, settings) {
  ...
  if (s.black_wolf) roles.push(ROLES.BLACK_WOLF);
  ...
  if (s.fool)       roles.push(ROLES.FOOL);
  if (s.elder)      roles.push(ROLES.ELDER);
  if (s.arsonist)   roles.push(ROLES.ARSONIST);
...
function buildNightQueue(round, roomSettings, gamePlayers) {
  ...
  // Sói Đen có lượt convert mỗi đêm
  if (s.black_wolf) queue.push(NIGHT_PHASES.BLACK_WOLF);

  // Kẻ Phóng Hỏa hành động mỗi đêm
  if (s.arsonist)   queue.push(NIGHT_PHASES.ARSONIST);
...
const ROLE_META = {
  ...
  [ROLES.FOOL]:       { name: 'Thằng Ngốc',         icon: '🤡',  team: TEAMS.SOLO,     nightAction: false },
  [ROLES.ELDER]:      { name: 'Già Làng',           icon: '👴',  team: TEAMS.VILLAGE,  nightAction: false },
  [ROLES.ARSONIST]:   { name: 'Kẻ Phóng Hỏa',      icon: '🔥',  team: TEAMS.SOLO,     nightAction: true  },
  [ROLES.BLACK_WOLF]: { name: 'Sói Đen',            icon: '🖤',  team: TEAMS.WEREWOLF, nightAction: true  },
};
